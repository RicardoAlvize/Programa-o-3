...
function constroiPagina(dados) {
  $ajaxUtils.sendGetRequest(itensHtml, function() { geraPaginaPrincipal(dados, itensHtml) }, false); // não é um JSON
}
...
function geraPaginaPrincipal(dados, itensHtml) {
    ...
    for (var i = 0, max = dados.length; i < max; i++) {
        ...
        htmlFinal += html;
		menu += '<li><a href="' + titulo + '">' + titulo + '</a></li>';
    }
    htmlFinal += '</section>';
    insereHtml(".container", htmlFinal);
}
